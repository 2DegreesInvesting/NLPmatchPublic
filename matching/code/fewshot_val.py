...
def get_data(input_file_name, file_path):
    ...
    train = pd.read_csv('matching/data/input/20240603_training_data_full.csv')[['clustered', 'ecoinvent_prod', 'manual_match_certainty']].rename(columns = {'manual_match_certainty':'Result'})
    # train = pd.read_csv('_00_production/data/dpr_training/3010_train_positive_answers.csv')[['clustered', 'ecoinvent_prod', 'manual_match_certainty']].rename(columns = {'manual_match_certainty':'Result'})
...
def main():
    ...
    get_batch_output(data_list, output_file_path, few_shot_prompt_template)
